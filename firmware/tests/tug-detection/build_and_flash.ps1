...
if (-not (Test-Path $ConfigFile)) {
    Write-Error "sdk.local.ps1 not found. Copy firmware/sdk.local.ps1.example to firmware/sdk.local.ps1."
    exit 1
}
. $ConfigFile
...
$OcdCfg   = "..\..\openocd.cfg"
$Elf      = ($PSScriptRoot -replace '\\', '/') + "/build/tug_detection_test.elf"

$env:PATH = "$MsysBase\bin;$env:PATH"
